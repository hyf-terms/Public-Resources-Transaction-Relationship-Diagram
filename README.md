...
当前版本首先实现中国政府采购网公开公告的低频采集，并已完成 P0 数据正确性改造：原始文档版本化、项目根/采购轮次/标包拆分、追加式事件、金额类型和可审计关联。代码仅使用 Python 标准库。
...
- P0 的标题辅助匹配分数仍是启发式规则，尚未经过人工标注集校准；
- 标包号提取仍是基础规则，复杂多包表格需要在 P1 增强；
- 多供应商总金额在正式数据库中标记为未分配；旧版供应商CSV仍采用均分，仅可用于初筛；
- 尚未实现供应商、品牌、上市公司和时点有效股权关系图。

下一阶段 P1 将建立300—500条人工标注链，重点提升招标—中标—合同的标包级匹配精度。
